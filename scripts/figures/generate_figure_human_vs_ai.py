...
def main():
    ...
    output_dir = Path(__file__).parent.parent.parent / "paper" / "supplementary"
    output_dir.mkdir(parents=True, exist_ok=True)

    # Save figure
    output_path = output_dir / 'figure_human_vs_ai_comparison.png'
    ...
    fig.savefig(extra_plots_dir / "figure_S9_human_vs_ai.png", dpi=300, bbox_inches='tight', facecolor='white')

    # Save data summary
    data_path = extra_plots_dir / "figure_S9_human_vs_ai_data.txt"
    with open(data_path, "w") as f:
        f.write("Figure S9: Human vs AI Error Rates by Flag Accuracy Criterion\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"{'Criterion':<25s} {'Human Error%':>12s} {'Best AI Error%':>15s} {'Worst AI Error%':>16s} {'Best AI Model':<35s}\n")
        f.write("-" * 103 + "\n")
        task_labels = {'affiliation': 'Inst. Affiliation', 'institution': 'Institution Type', 'domain': 'Email Domain', 'sanctions': 'Sanctions'}
        for task in ['affiliation', 'institution', 'domain', 'sanctions']:
            if task in results:
                d = results[task]
                f.write(f"{task_labels[task]:<25s} {d['human_error']:12.1f} {d['best_error']:15.1f} {d['worst_error']:16.1f} {d['best_model']:<35s}\n")
