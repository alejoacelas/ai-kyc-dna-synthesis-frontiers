#!/usr/bin/env python3
"""
Generate Figure 1b: Pass rates heatmap by screener and task.
...
X-axis shows tasks: Institutional Affiliation, Institution Type, Email Domain,
Sanctions Screening, Relevant Work.
"""
...
# Task mapping from metric_name prefix to display label
TASK_ORDER = [
    "affiliation",
    "institution",
    "domain",
...
def load_and_filter_data():
    ...
    # Extract task from metric_name
    df_filtered['task'] = df_filtered['metric_name'].apply(extract_task)
    df_filtered = df_filtered[df_filtered['task'].notna()]
    print(f"Rows with valid task: {len(df_filtered):,}")
...
def create_figure(df):
    ...
    # Calculate pass rates by model and task
...
def main():
    """Main function to generate Figure 1b."""
    print("Generating Figure 1b: Pass rates heatmap by task")
    print("=" * 60)
    ...
    # Save the figure
    output_path = Path(__file__).parent.parent.parent / "paper" / "supplementary" / "figure2b_pass_rates_by_task.png"
    ...
    fig.savefig(extra_plots_dir / "figure_S1_pass_rates_by_task.png", dpi=300, bbox_inches='tight', facecolor='white')

    # Save data summary
    pivot = df.pivot_table(values="pass", index="model_label", columns="task", aggfunc="mean") * 100
    data_path = extra_plots_dir / "figure_S1_pass_rates_by_task_data.txt"
    with open(data_path, "w") as f:
        f.write("Figure S1: Pass Rates by Screener and Task (40-profile subset)\n")
        f.write("=" * 70 + "\n\n")
    ...
    print("\n" + "=" * 60)
    print("Caption:")
    print("Pass rates by screener and task. Tasks include: Institutional Affiliation,")
    print("Institution Type, Email Domain, Sanctions Screening, and Relevant Work.")
