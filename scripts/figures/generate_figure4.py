...
def main():
    """Main function to generate both versions of Figure 4."""
    print("Generating Figure 4: Cost vs Performance scatter plots")
    print("=" * 60)
    ...
    output_path_full = supplementary_dir / "figure4_cost_vs_performance_full.png"
    ...
    fig_full.savefig(extra_plots_dir / "figure_S7_cost_vs_performance_full.png", dpi=300, bbox_inches='tight', facecolor='white')

    # Save data summary for full dataset
    df_ai = df[df["model_type"] != "human_baseline"].copy()
    customer_costs = df_ai.groupby(["customer_name", "model_label"]).agg({
    ...
    agg["pass_rate"] = agg["num_assertions_passed"] / agg["num_assertions"] * 100
    data_path = extra_plots_dir / "figure_S7_cost_vs_performance_full_data.txt"
    with open(data_path, "w") as f:
        f.write("Figure S7: Cost vs Performance - Full Dataset (134 profiles)\n")
        f.write("=" * 70 + "\n\n")
        f.write(agg[["ai_cost", "pass_rate"]].round(3).to_string())
        f.write("\n")
    ...
    output_path_hbs = figures_dir / "figure4_cost_vs_performance.png"
