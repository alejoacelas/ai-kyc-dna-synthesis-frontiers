...
def load_data():
    ...
    # Load CSV files
    tests_df = pd.read_csv(BASE_DIR / "processed" / "tests.csv")
    responses_df = pd.read_csv(BASE_DIR / "processed" / "responses.csv")
    full_dataset_df = pd.read_csv(BASE_DIR / "data" / "customers" / "full_dataset.csv")

    # Load JSON file
    with open(BASE_DIR / "data" / "annotations" / "blind_gradings.json", 'r') as f:
